...
finishQuiz() {
                ...
                this.state.isQuizActive = false;
                this.unlockInterface();
                this.syncPracticeRecord(); //新增同步函數
                this.updateWrongQuestions();  //新增錯誤題目
                this.showResult();
...
// ✨✨✨ 新增數據同步核心函數 ✨✨✨
    async syncPracticeRecord() {
        ...
        if (!userId) {
            this.showAlert('❌ 警告：未登入，記錄將不會上傳雲端！', 'warning');
            return;
        }
        ...
        const answersData = Object.keys(this.state.userAnswers).map(index => {
            ...
            return {
                ...
                user_original_index: answer.originalIndex, // 增加原始索引
                is_correct: isCorrect,
                time: answer.timestamp.toISOString() // 使用 ISO 字串
            };
        });
        ...
        const syncPayload = {
            ...
            answers: answersData, // 傳送整理後的答案陣列
            markedQuestions: this.state.markedQuestions // 傳送標記的題目
        };

        // 2. 設定 n8n Webhook URL
        const webhookUrl = 'https://nickleo9.zeabur.app/webhook/save-practice-record'; 

        this.showLoading('同步練習記錄中...');
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.showAlert(`✅ 記錄同步成功！分數: ${syncPayload.score}/${syncPayload.questionCount} (${syncPayload.accuracy}%)`, 'success');
                console.log('✅ Supabase 同步結果:', result);
            } else {
                this.showAlert('⚠️ 雲端同步失敗，請檢查網路或後台服務。', 'error');
                console.error('❌ Supabase 同步錯誤:', response.status, await response.text());
            }

        } catch (error) {
            this.hideLoading();
            this.showAlert('🚨 網路錯誤，練習記錄未上傳。', 'error');
...
async updateWrongQuestions() {
    ...
    try {
        ...
        </script>  <!-- ← 這裡結束上一個 script -->
    <!-- ✨ 新增：登入功能 JavaScript START -->
    <script>
        // ==================== 登入管理系統 (LoginManager) ====================
